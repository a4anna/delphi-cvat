...
class CLI():
    ...
    def tasks_create(self, name, labels, resources, **kwargs):
        ...
        self.tasks_data(response_json['id'], resources)
    ...
    def tasks_dump(self, task_id, fileformat="CVAT for images 1.1", **kwargs):
        ...
        annotations = ET.fromstring(zf.read("annotations.xml").decode('UTF-8'))
        image_ids = []
        for img in annotations.findall("image"):
            tag = img.find('tag')
            if tag is not None:
                # print(img.get("name"))
